...
namespace mpsxx {
    ...
    */
   template<class Q>
      MPS<Q> create(int L,const Q &qt,const Qshapes<Q> &qp,int D,const std::function<double(void)>& f_random_generator){ 

         //shape of the physical index
         Dshapes dp(qp.size(),1); // dim physical, qp.size() ints with value 1
         std::vector< Qshapes<Q> > qr(L); // quantum right size L, match each site
                                          // Qshapes<Q> is a vector of tuples, each tuple
                                          // denotes a possible combination of physical
                                          // indices (in this case only spin)
                                          // i.e. possible quantum numbers for right index of each matrix
         std::vector<Dshapes> dr(L); // dim right
                                     // Dshapes donates "dense shape". At each site, there is a Dshape,
                                     // specifying the right dimension associated with each right quantum
                                     // number.
         ...
         TVector<Qshapes<Q>,3> qshape; // Quantum number shape: left, physical, right
         TVector<Dshapes,3> dshape;  // dimension shape: left, physical, right
                                     // for each of them, eg. right, specify dimension associated
                                     // with each right quantum number

         //first 0
         Qshapes<Q> ql(1,Q::zero()); // 1 bond with 0
   ...
   /**
    * create an MPS chain of length L initialized randomly on total Q number qt, with physical quantumnumber qp, physical dimensions dp
    * @param L length of the chain
    * @param qt total quantumnumber
    * @param qp Qshapes object containing the physical quantumnumbers
    * @param dp Dshapes object containing the physical dimensions 
    * @param D maximal dimension of the quantum blocks
    * @param f_random_generator predefined function which calls a random number generator
    * @return the MPS chain randomly filled and with correct quantumnumbers and dimensions
    */
   template<class Q>
      MPS<Q> create(int L,const Q &qt,const Qshapes<Q> &qp,const Dshapes &dp, int D,const std::function<double(void)>& f_random_generator){ 
   ...
   template<size_t N, class Q>
     MPX<N,Q> conjugate(const MPX<N,Q>& A) {
       MPX<N,Q> B(A.size());
   ...
   template<class Q>
      void gemv(double alpha,const MPO<Q> &A,const MPS<Q> &X,double beta,MPS<Q> &Y){
         ...
         if(fabs(beta) < 1.0e-15){
            ...
            QSDArray<5> tmp;
            QSDArray<4> mrows;
            # pragma omp parallel default(none) shared(A, X, L, Y) private(tmp, mrows)
            {
 ...
 /**
   * calculate Schmidt coefficients of the MPS
   */
  template<class Q> 
  std::tuple<SDArray<1>, Qshapes<Q>> Schmidt(MPS<Q>& mps, int site = -1) {
      vector<double> coef;
      if (site < 0) {
        site = mps.size()/2;
      }
